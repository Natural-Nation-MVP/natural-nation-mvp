const STORAGE_KEY='founder-os-live-pilot-v1';
...
const initialState={
  ...
  workspaces:[
    {id:'workspace-natural-nation',number:1,name:'Natural Nation',state:'Pilot Ready',summary:'Duey wellness workflows operating through the Founder OS governed runtime.',monthlyCost:42.18,
      ...
      schedules:[{title:'Daily protocol preparation',meta:'Configured · 6:00 AM workspace time'}],release:[{title:'Live Founder Review Pilot',meta:'UI ready · runtime connection verification pending'}],evidence:[{title:'pilot-baseline',meta:'Local review artifact · not production evidence'}]},
    {id:'workspace-contractor-estimator',number:2,name:'Contractor Estimator',state:'Isolated',summary:'Independent validation workspace for cross-workspace isolation tests.',monthlyCost:3.12,runs:[],approvals:[],health:[{title:'Workspace boundary',meta:'Ready for denial test'}],schedules:[],release:[{title:'No active release',meta:'Workspace remains isolated'}],evidence:[]}
  ],
...
function render(){
  ...
  ['runs','approvals','health','schedules','release','evidence'].forEach((key)=>byId(key).innerHTML=renderItems(w[key],key));
...
function renderVerification(){
  const checks=[['pages','GitHub Pages application loads'],['api','Cloudflare runtime health responds'],['database','Runtime persists and reloads a pilot run'],['providers','OpenAI and Google AI report server-side health'],['workspace','Natural Nation remains Workspace #1'],['approval','Consequential action requires exact Founder approval'],['evidence','Run emits evidence and audit records'],['retry','Transient failure demonstrates bounded retry'],['isolation','Cross-workspace request is denied']];
...
async function connect(){
  const value=byId('api-base').value.trim();
...
async function refreshRuntime(){
  ...
  try{
    ...
    if(Array.isArray(data?.workspaces)&&data.workspaces.length)state.workspaces=data.workspaces;
    byId('notice').textContent='Live runtime connected. Data was refreshed from the governed API.';
  }catch(error){state.mode='review';state.verification.api=false;byId('notice').textContent=`Review mode active: ${error.message}. The UI remains testable, but generated records are local review evidence only.`}
  state.verification.pages=true;render();
}

async function runPilot(){
  if(state.selectedWorkspaceId!=='workspace-natural-nation'){byId('notice').textContent='Select Natural Nation before running the primary pilot.';return}
  byId('run-pilot').disabled=true;byId('notice').textContent='Starting governed pilot…';
  try{
    if(state.mode==='live'){
      const result=await api('/api/founder-os/pilot/run',{method:'POST',body:JSON.stringify({workflow:'natural-nation-founder-review',requestedAt:now()})});
      applyLiveResult(result);
    }else simulatePilot();
  }catch(error){byId('notice').textContent=`Pilot request failed safely: ${error.message}`}
  finally{byId('run-pilot').disabled=false;render()}
}

function simulatePilot(){
  const w=workspace();const runId=uid('run-nn');const approvalId=uid('approval');
  ...
  w.approvals.unshift({id:approvalId,runId,title:'Publish pilot protocol artifact',meta:'Founder approval required · exact payload hash bound',hash:`sha256:${uid('payload')}`});
  ...
  byId('notice').textContent='Review pilot completed. A consequential publication is waiting for exact Founder approval.';
}

function applyLiveResult(result={}){
  const w=workspace();if(result.run)w.runs.unshift(result.run);if(result.approval)w.approvals.unshift(result.approval);if(result.evidence)w.evidence.unshift(...result.evidence);
  ...
  byId('notice').textContent='Live pilot completed. Review the returned evidence and any pending approval.';
}
...
async function approvePending(){if(!pendingApproval)return;const w=workspace();
