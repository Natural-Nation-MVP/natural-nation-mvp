(() => {
  ...
  const isGatewayPreview = /founder-os-gateway/i.test(window.location.hostname) && /\.workers\.dev$/i.test(window.location.hostname);
  const gatewayOrigin = isGatewayPreview ? window.location.origin : productionGatewayOrigin;
