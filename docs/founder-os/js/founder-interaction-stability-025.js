(() => {
  ...
  function syncWorkspaceNavigation(target = document.body.dataset.activeView || 'mission') {
    ...
    if (stale) {
      ...
      nav.innerHTML = `<button class="nav-link back-link" type="button" data-command-center-home>← Founder OS Home</button><div class="nav-context"><small>You are working in</small><strong>${escapeHtml(workspace.name)}</strong><span>${escapeHtml(workspace.roleLabel || workspace.type || 'Workspace')}</span></div>${[...groups.entries()].map(([group, modules]) => `<div class="nav-group"><div class="nav-group-label">${escapeHtml(group)}</div>${modules.map((module) => `<button class="nav-link${module.target === target ? ' active' : ''}" type="button" data-context-module="${escapeHtml(module.target)}">${escapeHtml(module.label)}</button>`).join('')}</div>`).join('')}`;
    }
  ...
  function removeRedundantOpenLinks() {
    $$('.workspace-card [data-resume-workspace]').forEach((button) => {
      button.hidden = true;
  ...
  window.addEventListener('founder-os:workspace-view-changed', (event) => {
    const target = event.detail?.target || document.body.dataset.activeView || 'mission';
    if (event.detail?.workspace) requestAnimationFrame(() => syncWorkspaceNavigation(target));
    else requestAnimationFrame(stabilizeHome);
  });
