(() => {
  ...
  function render(){
    ...
    root.innerHTML=`<div class="knowledge-shell">
      <section class="glass-panel knowledge-command"><div><div class="eyebrow">Workspace Knowledge</div><h2>Knowledge Records</h2><p class="muted">Approved information and AI-prepared drafts for ${escapeHtml(workspaceName())}.</p></div><div class="knowledge-command-actions"><input class="knowledge-search" type="search" placeholder="Search records" value="${escapeHtml(state.query)}" data-knowledge-search><button class="btn secondary" type="button" data-knowledge-action="audit">Audit Records</button><button class="generate" type="button" data-knowledge-action="new-draft">New Draft +</button></div></section>
      ...
      <div class="knowledge-feedback" data-knowledge-feedback data-knowledge-action-output aria-live="polite"></div>
