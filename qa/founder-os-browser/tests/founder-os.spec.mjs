...
test('mobile Build and Team routes open on the first tap after refresh', async ({ page }, testInfo) => {
  ...
  await expect.poll(() => page.locator('[data-workspace="build"]').evaluate((element) => Math.abs(element.getBoundingClientRect().top))).toBeLessThanOrEqual(2);
  ...
  await expect(page.locator('[data-workspace="ai"]')).toBeVisible();
  await expect.poll(() => page.locator('[data-workspace="ai"]').evaluate((element) => Math.abs(element.getBoundingClientRect().top))).toBeLessThanOrEqual(2);
