  ...
  return (
    <SafeAreaView style={{ backgroundColor: missionControlTheme.colors.background, flex: 1 }}>
      ...
      <View style={{ backgroundColor: missionControlTheme.colors.background, flex: 1, flexDirection: 'row' }}>
        <View style={{ backgroundColor: '#06101b', borderRightColor: missionControlTheme.colors.border, borderRightWidth: 1, maxWidth: 340, minWidth: 280, padding: 18 }}>
          <Text style={{ color: missionControlTheme.colors.emerald, fontSize: 13, fontWeight: '900', letterSpacing: 2 }}>NNCC</Text>
          <Text style={{ color: missionControlTheme.colors.textPrimary, fontSize: 25, fontWeight: '900', marginTop: 5 }}>Founder OS</Text>
          <Text style={{ color: missionControlTheme.colors.textMuted, fontSize: 12, lineHeight: 18, marginBottom: 18, marginTop: 6 }}>Powered by NNOS</Text>
          <ScrollView showsVerticalScrollIndicator={false}>
            {nnccWorkspaceNavigation.map((group) => (
              <View key={group.id} style={{ marginBottom: 18 }}>
                <Text style={{ color: getGroupAccent(group.accent), fontSize: 11, fontWeight: '900', letterSpacing: 1.6, marginBottom: 8 }}>
                  {group.label.toUpperCase()}
                </Text>
                {group.routes.map((item) => {
                  const isActive = item.key === activeRoute;
                  return (
                    <Pressable key={item.key} onPress={() => setActiveRoute(item.key as RouteKey)} style={{ backgroundColor: isActive ? 'rgba(34, 245, 169, 0.14)' : 'transparent', borderColor: isActive ? missionControlTheme.colors.emerald : 'transparent', borderRadius: 14, borderWidth: 1, marginBottom: 8, padding: 12 }}>
                      <Text style={{ color: missionControlTheme.colors.textPrimary, fontSize: 14, fontWeight: '900' }}>{item.label}</Text>
                      <Text style={{ color: missionControlTheme.colors.textMuted, fontSize: 11, lineHeight: 16, marginTop: 4 }}>{item.description}</Text>
                    </Pressable>
                  );
                })}
              </View>
...
function renderPage(route: RouteKey) {
  ...
  if (route === 'readiness') return <ReleaseReadinessView />;
