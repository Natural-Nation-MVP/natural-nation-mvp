...
const executionPackage=json('docs/execution-packages/NN-BUILD-001.json');
const orchestrationState=json('docs/founder-os/config/ai-orchestration-state.json');
...
assert(registry.items.some((item)=>item.status==='complete'&&item.backlogId==='NN-BACKLOG-001'));
assert.equal(executionPackage.status,'ready');
assert.equal(orchestrationState.status,'complete');
assert.equal(orchestrationState.orchestrationCycleStatus,'complete');
assert.equal(orchestrationState.packageStatus,executionPackage.status);
assert.equal(orchestrationState.packageDelivery.status,'not-delivered');
