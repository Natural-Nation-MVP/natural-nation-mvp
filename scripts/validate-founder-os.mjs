...
const files = await Promise.all([
  ...
  read('docs/founder-os/js/repository-intelligence.js'),
  read('docs/founder-os/css/repository-actions.css'),
...
assert(repositoryActions.includes('mergeComplete'));
assert(repositoryActions.includes('deploymentComplete'));
assert(repositoryActions.includes('renderCompletedState'));
assert(repositoryActions.includes('Merge handoff completed'));
assert(repositoryActions.includes('Open merged pull request'));
assert(repositoryActions.includes('Open merge commit'));
assert(repositoryActions.includes('Repository lifecycle'));
...
assert(githubLibrary.includes('fileId: `FILE-'));
assert(githubLibrary.includes('crypto.subtle.digest("SHA-256"'));
assert(githubLibrary.includes('fingerprint: await fingerprint'));
...
for (const field of ['mergeBlockers', 'recommendation', 'evidence', 'nextAction', 'fileId', 'fingerprint']) assert(providerContracts.includes(field), `Missing structured field: ${field}`);
assert(providerContracts.includes('evidenceItems(dispatch)'));
assert(providerContracts.includes('Never return repository paths in evidence'));
...
assert(providerAdapters.includes('evidenceContract: dispatch.evidenceContract'));
assert(providerAdapters.includes('INVALID_RESPONSE'));
assert(resultVerification.includes('evidenceMap'));
assert(resultVerification.includes('unknown evidence ID'));
assert(resultVerification.includes('fingerprint mismatch'));
