...
for (const deleted of ['queue-meta.js', 'command-engine.js', 'workspace-blueprint.js']) {
  assert(!html.includes(deleted), `Deleted legacy runtime must not load: ${deleted}`);
}
...
for (const source of ['processing-status.js?v=processing-v1', 'canonical-build-studio.js?v=processing-v1', 'ai-orchestration.js?v=processing-v1', 'build-dispatch-bridge.js?v=processing-v1']) {
  assert(moduleLoader.includes(source), `Current execution module must be cache-busted: ${source}`);
}
...
for (const requirement of ['data-processing-status', 'data-processing-elapsed', 'aria-live', 'processing-active']) {
  assert(processingStatus.includes(requirement), `Processing status requirement is missing: ${requirement}`);
}
assert(dispatchBridge.includes('monitorCanonicalState'), 'Build Work must monitor canonical state during long provider requests.');
assert(dispatchBridge.includes('LONG_RUNNING_NOTICE_MS'), 'Long provider requests must transition to a still-working notice.');
assert(!dispatchBridge.includes('did not finish within 90 seconds'), 'A 90-second browser timer must not report a false task failure.');
assert(dispatchBridge.includes('hasTaskAdvanced'), 'Canonical completion or blocking must be the processing source of truth.');
...
for (const requirement of ['PROVIDER_REGISTRY', 'FAILOVER_CATEGORIES', 'providerOrder', 'fallbackUsed', 'attempts', 'QUOTA_EXCEEDED', 'AUTHENTICATION_FAILED', 'OPENAI_API_KEY', 'GOOGLE_AI_API_KEY', 'api.openai.com/v1/responses', 'generativelanguage.googleapis.com', 'synchronous: true', 'delivery-failed']) {
  assert(providerAdapters.includes(requirement), `Provider adapter requirement is missing: ${requirement}`);
}
